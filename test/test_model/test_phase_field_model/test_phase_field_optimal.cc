...
/* -------------------------------------------------------------------------- */
Real computeOptimalDamage(Real const & x, Real const & l, Real const & L) {
  Real optimal_damage = 0.;
  optimal_damage += std::exp(x / l) / (std::exp(2. * L / l) + 1.);
  optimal_damage +=
      std::exp(-x / l) * std::exp(2. * L / l) / (std::exp(2. * L / l) + 1.);
...
int main(int argc, char * argv[]) {
  ...
  initialize("material_optimal.dat", argc, argv);
  ...
  for (Int n = 0; n < phase.getMesh().getNbNodes(); ++n) {
    ...
    analytical_damage = computeOptimalDamage(positions(n, 0), l0, L);
